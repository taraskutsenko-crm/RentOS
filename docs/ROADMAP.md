...
| Pre-Chapter 10 — Rental Workflow, Contract System & No-Code Document Template Builder                           | COMPLETED | Fixed the blank-contract bug (rentalId/quoteId linking), document-version immutability pinning, `DocumentTemplate.language` per-language templates, Company Profile settings, `DatePicker`/`TimePicker`/`DateTimeField`, tenant-timezone display formatting, an 18-section default Rental Contract template, and a Tiptap-based no-code template builder (insert-field chips, section library, draft preview against synthetic data, starter template) with `getRentalNextAction()`/`getQuoteNextAction()` workflow-continuity. See D-060–D-064. |
...
| Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Status            |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- |
| Rental customer invoicing — foundational pass shipped PRE-CHAPTER-10 (Invoice/Payment/CompanyBankAccount models, Rental→Invoice prefill, multi-invoice-per-rental, payment tracking, Polish/English rendering, provider-neutral e-invoice boundary with a KSeF settings stub — see D-081 through D-089 in [DECISIONS.md](DECISIONS.md)); correction/credit-note workflow, dashboard financial widgets, tenant-configurable numbering formats, and real KSeF connectivity remain unbuilt. Deposit collection/refund workflows (distinct from TASK-0011's SaaS subscription billing — see that task's note above) remain PLANNED | IN PROGRESS       |
