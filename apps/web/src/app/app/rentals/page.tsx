...
export default function RentalsPage() {
  ...
  const { data, isLoading, isError } = useRentals(tenantId, {
    page,
    pageSize: PAGE_SIZE,
      ...
      <Card>
        <CardContent className="p-0">
          ...
          {!isLoading && data?.items.length === 0 && (
            <p className="text-muted-foreground p-6 text-sm">{t("rental.noRentals")}</p>
          )}
              ...
              <tbody>
                {data.items.map((rental) => (
                  <tr key={rental.id} className="border-b last:border-0">
                    ...
                    <td className="p-3">{t(`rental.statuses.${rental.status}`)}</td>
                    ...
                    <td className="p-3 text-right">
                      <Button asChild variant="outline" size="sm">
                        <Link href={`/app/rentals/${rental.id}`}>{t("asset.view")}</Link>
                      </Button>
                    </td>
                  </tr>
                ))}
