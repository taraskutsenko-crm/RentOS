...
All authenticated endpoints live under `/tenants/:tenantId/documents` (see
[ADR 0010](adr/0010-document-management-platform.md) for the domain model
and [ADR 0011](adr/0011-document-rendering-and-sharing.md) for rendering,
templates, sharing, email, and e-signature). Require authentication +
active membership (`TenantGuard`) plus the permission noted per endpoint,
except the `/public/documents/:token` routes documented at the end of this
section, which require neither.
...
- `GET .../:id/preview` — Requires `documents.render`. Returns
  `{ documentNumber, documentType, title, status, html, templateId,
templateSource }` — a live HTML render of the document's current
  version, computed fresh every call (never stored, see ADR 0011).
  `templateSource` is `explicit`\|`tenant_active`\|`built_in_default`,
  reflecting which resolution step produced the template used.
- `GET .../:id/pdf` — Requires `documents.download`. Serves the
  most-recently-generated PDF for the current version, generating one on
  first request. Each call is recorded as a `document.downloaded` timeline
  entry.
- `POST .../:id/pdf` — Requires `documents.render`. Forces a fresh PDF
  generation (a new `DocumentFile` row), returning the regenerated bytes.

### Document templates — `/tenants/:tenantId/document-templates`

| Endpoint                                                          | Requires                     | Notes                                                                                                         |
| ----------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `POST /tenants/:tenantId/document-templates`                      | `documents.templates.manage` | Creates a `DRAFT` template with its first version (`htmlContent` required, `css`/`variablesSchema` optional). |
| `GET .../document-templates`                                      | `documents.templates.view`   | Paginated list; filters `documentType`/`status`/`search`.                                                     |
| `GET .../document-templates/:id`                                  | `documents.templates.view`   | Full detail including every `DocumentTemplateVersion`.                                                        |
| `PATCH .../document-templates/:id`                                | `documents.templates.manage` | Metadata only (`name`/`description`) — never creates a new version.                                           |
| `POST .../document-templates/:id/versions`                        | `documents.templates.manage` | Always creates a new `DocumentTemplateVersion` — content is never edited in place.                            |
| `POST .../document-templates/:id/versions/:versionNumber/restore` | `documents.templates.manage` | Makes an older version's content the current one again (as a **new** version, not a rewind).                  |
| `POST .../document-templates/:id/activate`                        | `documents.templates.manage` | `DRAFT`/`ARCHIVED` → `ACTIVE`; demotes the tenant's previous `ACTIVE` template of the same type.              |
| `POST .../document-templates/:id/archive`                         | `documents.templates.manage` | → `ARCHIVED`.                                                                                                 |
| `POST .../document-templates/:id/restore`                         | `documents.templates.manage` | `ARCHIVED` → `DRAFT`.                                                                                         |
| `POST .../document-templates/:id/duplicate`                       | `documents.templates.manage` | Creates a new `DRAFT` template with a copy of the current version's content.                                  |

### Public sharing — authenticated management under `/tenants/:tenantId/documents/:id/share`

- `POST .../:id/share` — Requires `documents.share`. Body:
  `{ expiresInDays? (default 30, max 365), password? }`. Creates a link for
  the document's **current** version and disables any previously active
  link for the same document. **201** → `{ shareLink, token }` — `token`
  is the plaintext value, returned exactly once and never persisted (only
  its SHA-256 hash is stored).
- `GET .../:id/share` — Requires `documents.share`. Lists every link ever
  created for the document (active and disabled), each with
  `viewCount`/`downloadCount`/`lastAccessedAt`/`lastAccessedIp`.
- `DELETE .../:id/share/:shareLinkId` — Requires `documents.share`.
  Disables the link. **404** if it's already disabled or doesn't exist.

### Public document access — `/public/documents/:token` (no auth, `@Public()`)

Both routes are `POST`, not `GET` — a password (when the link requires one)
must travel in the request body, never a query string; see ADR 0011.

- `POST /public/documents/:token/view` — Body: `{ password? }`. Returns
  `{ documentNumber, documentType, title, status, html }`. **404** for an
  unknown/expired/disabled token. **403** if a password is required but
  missing or incorrect (both cases return the same generic messages, never
  distinguishing which, to avoid leaking token/password validity). A
  successful view also advances `Document.status` `SENT → VIEWED`, the same
  as the authenticated `markViewed` action.
- `POST /public/documents/:token/pdf` — Body: `{ password? }`. Streams the
  document's PDF (`Content-Disposition: attachment`), generating one if
  none exists yet. Same 404/403 semantics as `/view`.

Rate limits (`@Throttle`): 20/min for `/view`, 10/min for `/pdf`.

### Email delivery — `/tenants/:tenantId/documents/:id/email`

- `POST .../:id/email` — Requires `documents.send`. Body:
  `{ recipientType: CUSTOMER|EMPLOYEE|CUSTOM, customEmail? (required and only allowed when CUSTOM), subject, message? }`.
  Resolves the actual recipient address from the linked `Customer`/employee
  `User` for `CUSTOMER`/`EMPLOYEE`, sends synchronously via the shared
  `EmailProvider` (ADR 0007), and creates a `DocumentEmailDelivery` row
  (`status: SENT`\|`FAILED`).
- `GET .../:id/email` — Requires `documents.send`. Lists every delivery
  attempt for the document, newest first.
- `POST .../:id/email/:emailDeliveryId/retry` — Requires `documents.send`.
  Re-sends to the same recipient; only valid for a `FAILED` delivery.

### E-signature — `/tenants/:tenantId/documents/:id/signature-requests`

No real signature provider is integrated — every request is served by
`LocalMockSignatureProvider` (`provider: LOCAL_MOCK`). See ADR 0011 for the
`DocumentSignatureProvider` abstraction this is built on.

- `POST .../:id/signature-requests` — Requires `documents.sign`. Body:
  `{ signerName?, signerEmail? }`. Creates a request in status
  `REQUESTED`.
- `GET .../:id/signature-requests` — Requires `documents.sign`. Lists every
  request for the document.
- `POST .../:id/signature-requests/:signatureRequestId/refresh` — Requires
  `documents.sign`. Asks the bound provider for the current status
  (`LocalMockSignatureProvider` is a no-op here — status only changes via
  the document's own `sign`/`reject` actions).
- `POST .../:id/signature-requests/:signatureRequestId/cancel` — Requires
  `documents.sign`. → `CANCELLED`; only valid while `REQUESTED`/`PENDING`.
