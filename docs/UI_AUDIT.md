...
## Addendum — Documents & Contracts Workspace (Chapter 9)

### 57. The Document detail page is the last standalone entity page still missing `PageHeader`/a colored status badge — the same gap class Chapter 7/8 closed for Rentals/Quotes

`UI_RESEARCH.md` finding #66. The Documents _list_ page was never a
gap (already `PageHeader`+`DataTable`); only
`app/app/documents/[id]/page.tsx` hand-rolled a bare `<h1>`/`<p>`
header with plain colorless status text, exactly the shape
`RentalStatusBadge`/`QuoteStatusBadge` already solved twice.

### 58. `Asset.platformDocuments` and `Customer.documents` are real, unsurfaced FK relations — the third and fourth instance of the exact gap Chapter 7 first named for `Rental.documents`

`UI_RESEARCH.md` finding #65. Both relations are populated by the
same real `DocumentsService.create()` flow that already populates
`Rental.documents`/`Quote.platformDocuments`; neither
`AssetsService.findOne()` nor `CustomersService.findOne()` included
them before this chapter, so the Asset and Customer detail pages had
no way to show a customer's or asset's linked contracts/protocols even
though the backend already tracked the link.

### 59. No tenant-wide document-count aggregation endpoint exists — confirmed, and correctly so, since the Dashboard already established the project's answer to this exact gap

`UI_RESEARCH.md` finding #67. Building a new
`GET /documents/summary` endpoint was considered and rejected in favor
of the already-proven `pageSize:1`-composition technique
`use-dashboard-stats.ts` uses for every other entity's Dashboard KPI
(`UI_REDESIGN_PLAN.md` Chapter 4, design decision 11) — reusing an
established pattern rather than introducing a second one for the same
kind of value.

### 60. No `Invoice`/`Payment`/`Transaction` model exists — an "Invoice" document type would be entirely fabricated

`UI_RESEARCH.md` finding #63, re-confirming finding #49's original
full-schema grep. Chapter 9's own brief explicitly lists Invoice as a
"potential" document category and explicitly forbids inventing
backend capability merely to make a label appear — documented as a
future product gap, not built.

### 61. "Signing" a document is a staff/customer-authenticated status flip, never a real e-signature — unchanged by this chapter, and correctly left unchanged

`UI_RESEARCH.md` finding #69. `LocalMockSignatureProvider` remains the
only implementation; no DocuSign/Adobe Sign/Autenti/eIDAS integration
exists. Chapter 9's own brief explicitly forbids building e-signature
infrastructure — the existing Signature card (already real, already
shipped) was carried into the new Workspace layout unchanged.
...
| Document detail page       | No `PageHeader`/Entity Summary, no colored status, no related-entities section (list page already had `PageHeader`)      | Chapter 9 — Documents Workspace: `PageHeader`, Related entities card (Customer/Rental/Quote/Asset), `DocumentStatusBadge` (list + workspace), real Smart Summary via `pageSize:1` composition                                                    |
| Document status rendering  | Plain colorless text everywhere                                                                                          | Chapter 9 — one reusable `DocumentStatusBadge`, applied consistently to list + workspace                                                                                                                                                         |
| Asset/Customer ↔ Documents | `Asset.platformDocuments`/`Customer.documents` real but unsurfaced                                                       | Chapter 9 — small additive `findOne()` extensions + a Documents card on both detail pages, mirroring the Rental/Quote pattern                                                                                                                    |
| Rental document checklist  | No derived "what's missing" intelligence anywhere                                                                        | Chapter 9 — a pure, real-data-only checklist card (Commercial Offer/Contract/Handover/Return), never a new persisted state                                                                                                                       |
